% Define the piecewise potential energy function (Example 3)
g = @(x) (x >= 0).*sqrt(x) + (x < 0).*(0.01.*x.^10);
...
title('Numerical Evaluation of A(a) - Example 3');
text(aMin, areaMin + 0.05, sprintf('  Global Min: c \\approx %.6f', aMin), 'Color', 'red');